    ...
    <Card className="bg-surface border-white/5 col-span-1 md:col-span-3 lg:col-span-4">
      <CardHeader>
        <CardTitle className="text-white/60 text-sm font-mono uppercase tracking-widest flex items-center gap-2">
          <History size={16} className="text-primary" />
          Analysis History
          <span className="ml-auto text-white/20 text-[10px]">({runs.length})</span>
        </CardTitle>
        <CardDescription className="text-[10px] text-white/30 italic">Historical analysis snapshots for this repository.</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody className="text-xs">
              {runs.map((run, i) => {
                ...
                return (
                  <tr key={run.id ?? i} className="group hover:bg-white/5 transition-colors cursor-pointer border-b border-white/5 last:border-0 relative">
                    ...
                    <td className="py-4 pr-4 text-white/40 font-mono text-[10px]">
                      {run.branch ?? "—"}
                    </td>
                    ...
                    <td className="py-4 pr-4">
                      <div className="flex flex-col items-center gap-1">
                        <span className="font-bold text-white tracking-widest">{score}</span>
                        <div className="w-20 h-1 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className={cn("h-full transition-all duration-1000", score > 80 ? "bg-secondary" : score > 60 ? "bg-primary" : "bg-destructive")}
                            style={{ width: `${score}%` }}
                          />
                        </div>
                      </div>
                    ...
                    <td className="py-4 pr-4 text-center">
                      <span className={cn(
                        "font-mono font-bold",
                        techDebt < 30 ? "text-secondary" : techDebt < 50 ? "text-warning" : "text-destructive"
                      )}>
                        {techDebt}%
                      </span>
                    </td>
                    <td className="py-4 text-right">
                      <span className={cn(
                        ...
                        score > 80 ? "bg-secondary/10 text-secondary border-secondary/20" :
                        score > 70 ? "bg-primary/10 text-primary border-primary/20" :
                        "bg-white/5 text-white/40 border-white/10"
                      )}>
                        {run.overall_verdict ?? (score > 80 ? "Production Ready" : score > 70 ? "Stable" : "Audit Required")}
                      </span>
                    </td>
                  </tr>
                );
